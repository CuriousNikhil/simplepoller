...
import me.nikhilchaudhari.simplepoller.network.*
...
class ResponseImpl internal constructor(override val request: Request) : Response {
    ...
    override fun contentIterator(chunkSize: Int): Iterator<ByteArray> {
        return object : Iterator<ByteArray> {
            ...
            val stream =
                if (this@ResponseImpl.request.stream) this@ResponseImpl.raw else this@ResponseImpl.content.inputStream()
    ...
    internal companion object {
        ...
        -> Unit> = arrayListOf(
            ...
            { response, connection ->
                ...
                val input = data as? InputStream ?: return@arrayListOf
                ...
                input.use { input ->
                    connection.outputStream.use { output ->
                        while (input.available() > 0) {
                            output.write(
                                ByteArray(min(4096, input.available())).apply { input.read(this) }
                            )
                        }
                    }
                }
    ...
    var _history: MutableList<Response> = arrayListOf()
            ...
            if (this._connection == null) {
                this._connection = URL(this.request.url).openRedirectingConnection(
                    ...
                    (defaultStartInitializers + this@ResponseImpl.initializers +
                            defaultEndInitializers).forEach {
    ...
    override var encoding: Charset
        get() {
            ...
            this.headers["Content-Type"]?.let {
                val charset = it.split(";").asSequence().map { it.split("=") }
    ...
    // Initializers
    private val initializers: MutableList<(ResponseImpl, HttpURLConnection) -> Unit> = arrayListOf()
